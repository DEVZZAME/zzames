...
export const blogPosts: BlogPostItem[] = [
  {
    ...
    content: `정산 서비스에서 가장 먼저 부딪히는 문제는 원천 데이터가 하나의 기준으로 들어오지 않는다는 점입니다.
...
이 구조를 만들면 지급 로직을 수정할 때도 원천 수집기를 건드릴 필요가 줄어들고, 운영자가 어느 단계에서 문제가 생겼는지 더 쉽게 추적할 수 있습니다.`,
  },
  {
    ...
    content: `외부 시스템 연동은 단순 API 연동보다 인증 실패, UI 변경, 재시도 정책 같은 운영 이슈가 더 중요합니다.
...
이런 구조는 기술적으로 깔끔한 것보다 운영팀의 작업 시간을 줄이는 데 직접적인 효과가 있었습니다.`,
  },
  {
    ...
    content: `AI 도구를 잘 쓰는 핵심은 반복 작업만 넘기는 것이 아니라, 설계와 검증에도 끌어오는 것입니다.
